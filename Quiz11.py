...
def show_registration_form(language):
    ...
    canvas.create_text(screen_width//2, 100, text="Cadastro" if language == "pt" else "Registration", font=("Helvetica", 24), width=900, fill="black")

    labels = ["Nome:", "Email:", "Celular:", "Cidade:", "UF:", "Empresa:", "CNPJ:", "Segmento:"]
    labels_en = ["Name:", "Email:", "Phone:", "City:", "State:", "Company:", "CNPJ:", "Segment:"]
    y_positions = [200, 250, 300, 350, 400, 450, 500, 550]

    entries = []
    for idx, label in enumerate(labels):
        canvas.create_text(screen_width//2 - 200, y_positions[idx], text=labels[idx] if language == "pt" else labels_en[idx], font=("Helvetica", 18), width=200, fill="black", anchor="e")
        entry = tk.Entry(root, font=("Helvetica", 18))
        canvas.create_window(screen_width//2 - 150, y_positions[idx], window=entry, anchor="w")
        entries.append(entry)

    name_entry, email_entry, phone_entry, city_entry, uf_entry, company_entry, cnpj_entry, segment_entry = entries

    register_button = tk.Button(root, text="Cadastrar" if language == "pt" else "Register", font=("Helvetica", 18), command=save_registration_data, fg="white", bg="black", width=20, height=2)
