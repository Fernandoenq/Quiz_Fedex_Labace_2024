...
def show_correct_message(question_index):
    for widget in frame.winfo_children():
        if widget != rfid_label and widget != background_label:
            widget.destroy()
    message_label = tk.Label(frame, text=correct_messages[question_index], font=("Helvetica", 24), wraplength=700,
...
def show_incorrect_message(question_index):
    for widget in frame.winfo_children():
        if widget != rfid_label and widget != background_label:
            widget.destroy()
    message_label = tk.Label(frame, text=incorrect_messages[question_index], font=("Helvetica", 24), wraplength=700,
                             justify="center", bg="white")
    message_label.pack(pady=20)
    message_label = tk.Label(frame, text=message_after_reply[question_index], font=("Helvetica", 24), wraplength=700,
                             justify="center", bg="white")
...
def show_language_selection():
    # Carregar a imagem do logo com transparência

    icon_image=tk.PhotoImage(file='fedexLogo.png')
    tk.Label(frame, image=icon_image, bg="#00FFFFFF").place(x=300,y=300)
...
def show_final_message():
    for widget in frame.winfo_children():
        if widget != rfid_label and widget != background_label:
            widget.destroy()
    final_label = tk.Label(frame, text="Quiz Finished! / Quiz Finalizado!", font=("Helvetica", 24), wraplength=700,
                           justify="center", bg="white")
