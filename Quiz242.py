...
def show_final_message(in_time):
    ...
    if selected_language == "pt":
        if current_hits <= 3 and in_time is False:
    ...
    if in_time is True or current_hits <= 3:
        x1, y1 = screen_width // 2, screen_height // 2 - 180  # Inicio do preto
        x2, y2 = screen_width // 2 + 25, screen_height // 2 - 155  # Final do preto
        create_rounded_rectangle(canvas, x1, y1, x2, y2, radius=20, outline='red', width=2, fill='red')
...
def restart_quiz(event=None):
    ...
    rfid_data = ""
    start_quiz(selected_language)  # Reinicia o quiz na língua selecionada
...
def start_quiz(language):
    global questions, answers, correct_answers, correct_messages, incorrect_messages, message_after_reply, time_left, in_weight
    current_question = 0
    current_hits = 0
    time_left = 60  # Reinicia o temporizador para 60 segundos
    if language == "pt":
