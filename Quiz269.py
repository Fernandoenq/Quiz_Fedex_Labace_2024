...
def change_answer_bg_color_by_time(answer_index, temp_color):
    ...
    #root.after(8000)
    show_overlay_message(current_message, current_sub_message, current_is_correct)
    #root.after(8000, show_overlay_message(current_message, current_sub_message, current_is_correct))
...
def show_correct_message(question_index):
    ...
    #canvas.itemconfig(answer_bg_ids[current_answer - 1], fill="#4D148C")
...
def show_incorrect_message_by_timer(question_index):
    ...
    # Muda a cor de fundo da resposta incorreta para vermelho
    change_answer_bg_color_by_time(current_answer - 1, "#4d148c")  # Mudar a cor para vermelho
    print("current_answer")
    print(current_answer)
    print(current_answer - 1)
...
def read_rfid():
    # Declaração de variáveis globais usadas na função
    global rfid_data, current_question, current_answer, current_hits, rfid_allowed, time_left, is_paused
            ...
            while True:
                ...
                if ser.in_waiting > 0:
                    ...
                    if rfid_allowed:
                        ...
                        if cleaned_rfid_data == "UIDTAG:0415E795BE2A81":
                            current_answer = 1
                        elif cleaned_rfid_data == "UIDTAG:0423ED95BE2A81":
                            current_answer = 2
                        elif cleaned_rfid_data == "UIDTAG:042FF495BE2A81":
                            current_answer = 3
                        elif cleaned_rfid_data == "UIDTAG:0443FA95BE2A81":
                            current_answer = 4

                        # Verifica se o tempo restante é menor que 3 segundos e pausa o temporizador
                        if time_left < 3:
                            is_paused = True
...
def show_question(question, possible_answers, current_question, in_weight):
    ...
    logo_img = logo_img.resize((480, 147), Image.Resampling.LANCZOS)
...
def show_box_image():
    ...
    try:
        ...
        boximg = boximg.resize((388, 600), Image.Resampling.LANCZOS)
...
def show_overlay_message(message, sub_message, is_correct):
    global overlay_photo, QRCOde_img, QRCode_photo  # Adiciona a referência global para manter a imagem viva
    ...
    if current_question == 4:
        # Carrega o logo da FedEx, redimensiona e converte para o formato necessário.
        QRCOde_img = Image.open("QRCode.png").convert("RGBA")
        QRCOde_img = QRCOde_img.resize((480, 480), Image.Resampling.LANCZOS)
        QRCode_photo = ImageTk.PhotoImage(QRCOde_img)

        # Posiciona o logo da FedEx no centro da tela.
        canvas.create_image(screen_width // 2, screen_height // 2 + (screen_height // 4), image=QRCode_photo,
                            anchor="center")
    if current_question < 4:
...
def restart_quiz(event=None):
    ...
    current_question = 0
    current_hits = 0
    time_left = 40  # Reinicia o temporizador para 20 segundos
    rfid_data = ""
    is_paused = False
    stop_time_var = False
    rfid_allowed = False
...
def save_registration_data(language):
    ...
    data = {
        ...
        "Celular": phone_entry.get() if formatar_telefone(phone_entry.get()) == None else "",  # Obtém o valor do campo de entrada "Celular"
  ...
  # Obtém o valor do campo de entrada "CNPJ"
    ...
    # Verifica se o CNPJ está inválido e ajusta o valor
    if data["CNPJ"] == "CNPJ inválido":
        data["CNPJ"] = ""
    print(data)
...
def check_fields(language):
    ...
    if (language == "pt" and phone != "Celular" and match and phone != "") or (language == "en" and phone != "Phone" and phone != ""):
        validate_phone = True
    else:
        validate_phone = False

    validate_name = name != "Nome e Sobrenome" and name != "Name and Last Name" and name != ""
    validate_email = email != "Email" and email != "Email" and email != ""
    validate_city = city != "Cidade" and city != "City" and city != ""
    validate_uf = uf != "UF" and uf != "State" and uf != ""
    validate_company = company != "Empresa" and company != "Company" and company != ""
    validate_segment = segment != "Segmento" and segment != "Segment" and segment != ""
...
def key_pressed(char, language):
    # Insere o caractere pressionado na posição atual do cursor no campo de entrada ativo
    ...
    if char is not None:
        ...
        new_text = current_text[:cursor_index] + char + current_text[cursor_index:]
        active_entry.delete(0, tk.END)  # Apaga o texto atual do campo de entrada
        active_entry.insert(0, new_text)  # Insere o novo texto com o caractere adicionado
        active_entry.icursor(cursor_index + len(char))  # Move o cursor para após o novo caractere
...
def create_keyboard(root, canvas, language):
    ...
    y_offset = screen_height // 2 - 200  # Deslocamento inicial no eixo Y
    ...
    for i, key in enumerate(keys):
        ...
        # Define a ação do botão para inserir o caractere pressionado.
    ...
    #---------------------- Tecla de Backspace ---------------------------------
    # Calcula a posição para a tecla Backspace.
    x = x_offset + 9 * (button_width * 10 + button_spacing)
    y = y_offset + 4 * (button_height * 20 + button_spacing)

    # Cria o botão de Backspace separadamente.
    backspace_button = tk.Button(root, text="\u2190", font=button_font,
                                 command=lambda: backspace_pressed(language),
                                 width=button_width, height=button_height,
                                 bg=button_bg, fg=button_fg, bd=1, relief='raised')

    # Posiciona o botão de Backspace no canvas.
    canvas.create_window(x, y, window=backspace_button)
...
def show_rest_screen():
    ...
    # Carrega o logo da FedEx, redimensiona e converte para o formato necessário
    logo_img = Image.open("fedexLogo.png").convert("RGBA")
    logo_img = logo_img.resize((1000, 306), Image.Resampling.LANCZOS)
    logo_photo = ImageTk.PhotoImage(logo_img)
    # Posiciona o logo da FedEx no centro da tela
    canvas.create_image(screen_width // 2, screen_height // 3, image=logo_photo, anchor="center")
